        ...
        <nav class="space-y-1 text-sm">
            <div class="space-y-1 mb-5">
                <a href="{{ route('dashboard.articles.create') . '?categoryNumber=' . $categoryNumber }}" wire:navigate
                    class="flex gap-2 items-center px-3 py-2 rounded-md hover:bg-gray-200 {{ $active === 'newArticle' ? 'bg-gray-200' : '' }}">
                ...
                <a href="{{ route('dashboard.categories.create') }}" wire:navigate
                    class="flex gap-2 items-center px-3 py-2 rounded-md hover:bg-gray-200 {{ $active === 'newCategory' ? 'bg-gray-200' : '' }}">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
            ...
            <div
                onclick="window.location='{{ route('dashboard.articles.index', ['categoryNumber' => null]) }}'"
                class="relative group cursor-pointer w-full flex justify-between items-center px-3 py-2 rounded-md hover:bg-gray-200 {{ $active === 'allArticles' ? 'bg-gray-200' : '' }}">
            ...
            @foreach ($categories as $category)
                <div
                    onclick="window.location='{{ route('dashboard.articles.index', ['categoryNumber' => $category->category_number]) }}'"
                    class="relative group cursor-pointer w-full flex justify-between items-center px-3 py-2 rounded-md hover:bg-gray-200 {{ $active === 'category'.$category->category_number ? 'bg-gray-200' : '' }}">
